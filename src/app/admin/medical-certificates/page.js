...
const MedicalCertificatesListPage = () => {
  ...
  const fetchCertificates = async () => {
    setLoading(true);
    try {
      // If admin, get all. If specialist, get theirs. If patient, get theirs.
      // The backend route '/certificates/get-all/no-pagination' is admin only.
      // We might need a more role-specific fetch. For now, I'll use the one I know exists.
      let endpoint = "certificates/get-all/no-pagination";
      if (userRole === "specialist") {
         // Assuming we have a 'by-doctor' endpoint or something similar
         // endpoint = "certificates/by-doctor"; 
      }
      ...
      if (error.data) {
        console.log("DEBUG: Detected Role from Server:", error.data.detectedRole);
      }
