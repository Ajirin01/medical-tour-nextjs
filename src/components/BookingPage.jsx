      ...
      const loadRescheduleInfo = async () => {
        try {
          ...
          if (res) {
            ...
            // Limit slots to only this consultant
            const consultant = specialistsByCategory.find(s => s._id === res.consultant._id);
            if (consultant) setSpecialistsByCategory([consultant]);
          }
        } catch (err) {
