...
const AvailabilityCalendar = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (timeSlots.length === 0) {
      addToast('Please add at least one time slot', 'error');
      return;
    }

    setLoading(true);
    try {
      const response = await postData('availabilities/create/custom', {
        date: selectedDate,
        timeSlots,
        isRecurring,
      ...
      addToast(error.message || 'Error setting availability', 'error');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/2 space-y-6">
          <div className="bg-white dark:bg-gray-900 p-6 rounded-[32px] shadow-xl border border-gray-100 dark:border-gray-800 transition-all hover:shadow-2xl">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6 flex items-center gap-3">
              <FaCalendarAlt className="text-primary-6" /> Select Date
            </h2>
            <Calendar
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-[32px] shadow-xl border border-gray-100 dark:border-gray-800 transition-all hover:shadow-2xl">
            ...
            <label className="flex items-center gap-3 p-3 rounded-2xl hover:bg-blue-50 dark:hover:bg-gray-800 cursor-pointer transition-colors border-2 border-transparent hover:border-primary-5">
              <input
                type="checkbox"
                checked={isRecurring}
                onChange={(e) => setIsRecurring(e.target.checked)}
                className="w-5 h-5 text-primary-6 rounded-lg focus:ring-primary-5 bg-transparent"
              />
            ...
            {isRecurring && (
              <div className="mt-4 animate-fadeIn">
                ...
                <select
                  ...
                  className="w-full p-4 border-[3px] border-primary-5 rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary-6 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                >
        ...
        <div className="w-full md:w-1/2">
          <div className="bg-white dark:bg-gray-900 p-6 sm:p-8 rounded-[32px] shadow-xl border border-gray-100 dark:border-gray-800 h-full flex flex-col transition-all hover:shadow-2xl">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6 flex items-center gap-3 text-primary-9">
              <FaClock className="text-primary-6" /> Set Time Slots
            </h2>
            
            <div className="flex-1 space-y-4 mb-8 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
              {timeSlots.length === 0 ? (
                <div className="text-center py-12 bg-blue-50 dark:bg-gray-900 border-2 border-dashed border-primary-2 dark:border-gray-800 rounded-2xl">
                  <p className="text-primary-7 dark:text-primary-4 font-medium">No time slots added yet</p>
                ...
                timeSlots.map((slot, index) => (
                  <div key={index} className={`p-6 bg-white dark:bg-gray-800 rounded-[24px] border-2 transition-all hover:shadow-xl animate-slideIn group relative overflow-hidden ${
                    slot.hasClash ? 'border-red-400 bg-red-50/10' : 'border-primary-1 dark:border-gray-700 hover:border-primary-5'
                  }`}>
                    <div className={`absolute top-0 left-0 w-2 h-full ${slot.hasClash ? 'bg-red-500' : 'bg-primary-6'}`}></div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6 mt-2">
                      <div className="w-full sm:w-1/3">
                        <label className="text-xs font-black text-primary-9 dark:text-primary-4 uppercase tracking-wider mb-2 block ml-1">
                          <FaClock className="inline mr-1" /> Start Time
                        </label>
                        
                        {/* Custom Time Grid Picker */}
                        <div className="relative group/picker">
                          <div className="grid grid-cols-2 gap-1 p-1 bg-gray-50 dark:bg-gray-900/50 border-2 border-primary-1 dark:border-gray-700 rounded-2xl shadow-inner">
                            <select 
                              ...
                              className="bg-transparent font-black text-xl p-2 outline-none text-center appearance-none cursor-pointer hover:text-primary-6 dark:text-gray-100"
                            >
                            ...
                            <select 
                              ...
                              className="bg-transparent font-black text-xl p-2 outline-none text-center appearance-none cursor-pointer hover:text-primary-6 dark:text-gray-100 border-l border-primary-1 dark:border-gray-700"
                            >
                              {['00', '15', '30', '45'].map(m => (
                          ...
                          <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none text-primary-2 opacity-30">
                            <span className="text-[10px]">HH:MM</span>
                          </div>
                        </div>
                      </div>
                      
                      <div className="w-full sm:w-2/3">
                        <label className="text-xs font-black text-primary-9 dark:text-primary-4 uppercase tracking-wider mb-2 block ml-1">Select Duration</label>
                          ...
                          {durationOptions.map((opt) => (
                            <button
                              ...
                              className={`relative overflow-hidden py-4 rounded-2xl font-black text-sm transition-all border-2 flex flex-col items-center justify-center gap-1 ${
                                slot.duration === opt 
                                  ? 'bg-gradient-to-br from-primary-6 to-primary-8 text-white border-primary-6 shadow-lg scale-105 z-10' 
                                  : 'bg-white dark:bg-gray-800 text-gray-400 dark:text-gray-500 border-gray-100 dark:border-gray-700 hover:border-primary-3 hover:bg-primary-0'
                              }`}
                            >
                              <span className={`text-xl ${slot.duration === opt ? 'text-white' : 'text-primary-3'}`}>
                                {opt === 15 && '⚡'}
                                {opt === 30 && '🕒'}
                                {opt === 45 && '⏳'}
                              </span>
            ...
            <div className="space-y-4">
              <button
                type="button"
                onClick={addTimeSlot}
                className="w-full py-4 border-2 border-dashed border-primary-5 dark:border-primary-8 text-primary-7 dark:text-primary-4 font-bold rounded-2xl hover:bg-primary-1 dark:hover:bg-primary-9/20 transition-all flex items-center justify-center gap-2 group"
              >
              ...
              <button
                ...
                className="w-full py-5 bg-gradient-to-r from-primary-6 to-primary-8 text-white font-extrabold text-xl rounded-2xl shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all disabled:opacity-50 disabled:translate-y-0"
              >
