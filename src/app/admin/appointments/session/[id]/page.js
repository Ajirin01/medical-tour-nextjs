...
const SessionPage = () => {
  ...
  const handleEndSession = async () => {
    const currentAppointment = appointmentRef.current;
    if (!currentAppointment?.session?._id || !token || currentAppointment?.session?.appointment?.status === "completed") return;
    try {
 ...
 const handleAddReferral = async () => {
  ...
  const handleSessionEnded = useCallback(({ specialist, appointmentId }) => {
    try {
      ...
      // Ensure the logged-in user is either the patient or the specialist of this session
  ...
  }, [appointmentRef, session?.user?.id, handleEndCall, router, id, addToast]);
  ...
  return (
    <div className={`absolute top-0 left-0 w-full transition-all duration-300 z-9999999999`}>
      <div className="bg-black">
        ...
        {/* Patient-facing confirmation dialog (triggered by doctor's socket request) */}
        <ConfirmationDialog
