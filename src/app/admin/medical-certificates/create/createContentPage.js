...
const CreateMedicalCertificate = () => {
  ...
  const handleSessionChange = (e) => {
    ...
    if (selected) {
      setAppointment({
        ...
        patient: selected.user // Use the session's populated user as the patient
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Fetch specialist signature from session profile if possible, or assume it's in their profile
      ...
      const payload = {
        ...
        patient: appointment.patient?._id || appointment.user?._id || (typeof appointment.patient === 'string' ? appointment.patient : null) || (typeof appointment.user === 'string' ? appointment.user : null),
        doctor: appointment.consultant?._id || appointment.specialist?._id || (typeof appointment.consultant === 'string' ? appointment.consultant : null) || (typeof appointment.specialist === 'string' ? appointment.specialist : null),
        diagnosis,
        comment,
        certID,
        doctorSignature
